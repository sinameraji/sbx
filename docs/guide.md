...
1. **Bake a custom image** — extend `images/base/Dockerfile`, then `HOTCELL_IMAGE=my/hotcell:latest`. Fastest cold-start; engine-wide.
...
- **CLI harnesses:** run **OpenCode / Codex / Claude Code / pi.dev** (or any headless agent) in one command — e.g. [`examples/opencode.sh <repo> "<task>"`](../examples/opencode.sh) creates a sandbox, clones the repo, runs OpenCode headless via OpenRouter (your key stays on the engine), and destroys the sandbox when done.
...
- **Sandboxes** — create / list / stop / start / destroy; `/workspace` on a named volume that survives stop/start and engine restarts.
...
- **Resource limits** — hard per-sandbox **memory / CPU / PID** caps (cgroups), per create or as engine-wide defaults, so one sandbox can't starve the rest.
...
`npm run smoke` exercises the whole surface end-to-end against live Docker; `npm run smoke:py` does the same through the Python SDK; `npm run check:egress` unit-tests the egress control plane (no Docker); `npm run smoke:remote` drives a live remote engine via the SDK.
