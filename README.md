...
🌐 **[hotcell-bw3.pages.dev](https://hotcell-bw3.pages.dev/)** · Apache-2.0 · self-hosted · one daemon

```bash
npm install -g hotcell        # CLI (hotcell) + daemon (hotcelld) — npm publish pending; until then:
git clone https://github.com/sinameraji/hotcell && cd hotcell && npm install && npm run build
```
