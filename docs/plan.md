...
- [x] Per-sandbox metrics + cost meter. **Decision:** metrics come from the **Docker stats API** (`dockerode` `container.stats({stream:false})`), not direct cgroup-file reads — consistent with the Phase 0 Docker choice and working identically on Linux + macOS Docker Desktop (where host cgroup files aren't reachable). `stream:false` makes the daemon take two samples ~1s apart so `precpu_stats` is populated and the CPU% delta is correct from a single call. The driver gains `stats(id): SandboxStats` (CPU%, cumulative CPU-ns, online CPUs, resident mem with page-cache excluded, mem limit, net rx/tx, pids), normalized so future drivers report the same shape. An in-process **metrics sampler** (`src/metrics.ts`, `SBX_METRICS_INTERVAL_MS` default 10s — the cAdvisor-style collection the plan calls for, no per-sandbox sidecar) integrates cumulative usage per running sandbox: CPU as the reset-safe delta of cumulative CPU-ns, memory as resident-bytes × wall-clock seconds. Totals (`cpuSeconds`, `memByteSeconds`) live in a `usage` JSON column on the sandbox record, so they **persist across daemon restarts**. The **cost meter** (`src/cost.ts`) turns usage into a `{cpu, mem, total}` breakdown via configurable rates (`SBX_COST_CPU_PER_HOUR` default 0.05, `SBX_COST_MEM_GB_PER_HOUR` default 0.005). Exposed through REST (`GET /sandboxes/:id/metrics` → live snapshot + usage + cost; live is null when not running, and the read is passive — it neither counts as activity nor auto-resumes a paused sandbox), SDK (`Sandbox.metrics()`), and CLI (`sb stats <id>`). `npm run smoke` runs the sampler twice (advancing its clock) and asserts `cpuSeconds`/`memByteSeconds`/`cost.total` are all > 0 and a live snapshot is present. **Deferred within Phase 2:** per-sandbox egress byte metering (cost currently covers CPU + mem), structured logs, OpenTelemetry traces, the web dashboard, and the Python SDK.
