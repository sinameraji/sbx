...
> **Status:** Phases 0–2 complete; Phase 3 underway. The Docker container driver runs on Linux + macOS with durable state, idle auto-pause/resume, files/processes/sessions, a stateful code interpreter, preview URLs, backups, per-sandbox metrics + cost meter, structured logs + OpenTelemetry traces, API-key auth, a web dashboard with a **live terminal**, and the **egress control plane** below — including **default-deny egress on Linux**. Firecracker (Linux) / Apple Virtualization (macOS) microVM drivers are next and need a KVM/VZ host.
