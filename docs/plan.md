...
**Status: nearly complete.** Per-sandbox metrics from cgroups; cost meter ($ from CPU-sec/mem-GB-sec/egress with configurable rates); structured logs; OpenTelemetry traces (create→exec→destroy). Web dashboard: sandbox list, live xterm.js terminal, metrics, cost, preview URLs, kill/snapshot. Python SDK. **Done:** metrics + cost meter, egress metering, web dashboard, Python SDK, structured logs, OTel traces, API-key auth, and metrics history/sparklines. **Only the live xterm.js terminal** (needs a new interactive-exec driver primitive) **and the PyPI publish** (needs registry credentials) remain deferred.
